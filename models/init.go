...
func (d *GachaDB) GetUids() ([]string, error) {
	...
	err := d.db.Table("gacha_logs").Select("uid").Group("uid").Find(&result).Error
	if err != nil {
		return result, err
	}
	return result, nil
...
func (d *GachaDB) GetNumWithLast(uid, gachaType, id string) (int, error) {
	query := d.db.Table("gacha_logs").
		...
		Order("i DESC").
		Limit(100).
		Where("i<=(?)", d.db.Table("gacha_logs").Select("i").Where("uid=?", uid).Where("id=?", id))
...
func (d *GachaDB) GetUsedCost(uid string) ([]GachaUsedCost, error) {
	...
	for _, gachaType := range gachaTypes {
		query := d.db.Table("gacha_logs").
			...
			Where("uid=?", uid).
			Where("i>(?)",
				db.db.Table("gacha_logs").Where("uid=?", uid).Select("MAX(i)").Where("gacha_type=?", gachaType).Where("rank_type=5"))
		cost := 0
...
func (d *GachaDB) GetTotals(uid string) (map[string][]GachaTotal, error) {
	...
	rows, err := d.db.Table("gacha_logs").
...
func (d *GachaDB) GetLastIDs() (map[string]map[string]string, error) {
	...
	rows, err := d.db.Table("gacha_logs").Select("uid", "i", "gacha_type", "id").Order("i DESC").Group("uid").Group("gacha_type").Rows()
	if err != nil {
		return result, err
	}
	defer rows.Close()
